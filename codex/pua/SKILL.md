...
如果当前环境支持 sub-agent 或并行执行者，并且用户明确允许你委派工作，就切换到团队协作模式；否则不要假装已经完成了 delegation。

### Leader 规则

1. 派任务时附带完整上下文：目标、文件域、完成标准、禁区、已排除方案
2. 多个执行者并行时，明确隔离文件域，避免修改同一文件
3. 失败次数和已排除方案要沿任务传递，不能因为换了执行者就清零
4. 任何“已完成”声明都要回到主线程验收，靠命令输出和结果证据说话

### 执行者规则

1. 开工前先加载 PUA 方法论：三条铁律、五步方法论、7 项检查清单
2. 不等 Leader 催，自行根据失败次数升级强制动作
3. 卡在 L2+ 时，结构化汇报失败模式、已试方案、已排除项和下一假设
...
failure_mode: <原地打转|直接放弃|低质量完成|未搜索就猜|被动等待>
attempts: <已尝试方案>
excluded: <已排除可能性>
