...
現在の環境で sub-agent や並列実行者が使え、かつユーザーが明示的に委譲を許可した場合のみ、チーム運用モードに切り替える。使えない場合は、委譲済みであるかのように振る舞わないこと。

### Leader の規則

1. 目標、ファイル範囲、完了条件、禁則、排除済みアプローチをまとめて渡す
2. 並列実行時はファイル所有権を分離し、同一ファイルの同時編集を避ける
3. 担当替えしても失敗履歴は引き継ぐ。担当者を変えてもプレッシャーはリセットしない
4. 「完了しました」は証拠付きでのみ受理する

### 実行者 の規則

1. 開始前に PUA 方法論をロードする
2. 失敗回数に応じて自律的にエスカレーションする
3. L2+ では失敗モード、試行済みアプローチ、排除済み候補、次の仮説を構造化して報告する
...
failure_mode: <堂々巡り|放棄|品質不足|未検索の推測|受け身>
attempts: <試したアプローチ>
excluded: <排除済み候補>
